'use strict';
// ─── SEED ────────────────────────────────────────────────────────────
// Datos iniciales que toda instalación necesita para funcionar:
// - La OT "0000" de Trabajos Internos (y su cliente/unidad dummy).
// - El catálogo base de permisos + el rol "Administrador" con todos ellos.
// Todo acá es "INSERT OR IGNORE" / "si no existe, crear": correr esto
// dos veces no duplica nada.

const crypto = require('crypto');

async function inicializarTallerInterno({ run, get }) {
    try {
...
async function inicializarRolesYPermisos({ run, get }) {
    try {
...
module.exports = { inicializarTallerInterno, inicializarRolesYPermisos, PERMISOS_BASE };

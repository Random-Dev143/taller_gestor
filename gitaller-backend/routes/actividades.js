...
router.post('/orden/:ot', async (req, res) => {
    const { descripcion, tiempo_estimado, legajos_mecanicos, jefe_legajo, es_rutina } = req.body;
    ...
    await withTransaction(async () => {
        ...
        //
        // es_rutina solo es relevante para ot='0000' (tareas internas): 1 = rutina
        // (limpieza, capacitaciones — nunca se cierra), 0 = extraordinaria (se cierra
        // cuando termina, y cuenta para eficacia/eficiencia). Para OTs normales llega
        // undefined/false desde el frontend y se guarda como 0 sin ningún efecto.
        const result = await run(`INSERT INTO actividades (ot, descripcion, tiempo_estimado, legajo_mecanico, estado, es_rutina) VALUES (?, ?, ?, ?, 'Asignada', ?)`, [req.params.ot, descripcion, tiempo_estimado, legajos_mecanicos[0], es_rutina ? 1 : 0]);
...
router.post('/:id/estado', async (req, res) => {
    ...
    await withTransaction(async () => {
        ...
        // Las tareas internas rutinarias (limpieza diaria, herramientas
        // semanales, etc.) no tienen un fin definido: suceden, no se
        // miden contra un presupuesto. Nunca deben poder cerrarse desde
        // acá — de lo contrario habría que volver a crearlas a mano cada
        // vez. Esta validación es del backend a propósito (no alcanza con
        // ocultar el botón en el frontend): así queda garantizado pase lo
        // que pase del lado de la UI.
...
// mecánico se olvidó de cerrar (o que directamente no le corresponde cerrar
// a él). Cierra cualquier sesión "En Curso" que haya quedado abierta, marca
// a todo el equipo como Finalizada (para que deje de aparecer en la
// pantalla de cada mecánico) y deja la actividad en 'Cerrada por Jefe' —
// sincronizarEstadoActividad respeta ese estado como definitivo mientras
// nadie vuelva a poner algo en curso.
router.post('/:id/cerrar-jefe', async (req, res) => {
    await withTransaction(async () => {
        ...
        const equipo = await all(`SELECT legajo_mecanico, estado FROM actividad_mecanicos WHERE actividad_id = ?`, [req.params.id]);
        for (const miembro of equipo) {
            if (miembro.estado === 'En Curso') {
                const sesion = await get(`SELECT id, inicio FROM tiempos_actividad WHERE actividad_id = ? AND legajo_mecanico = ? AND fin IS NULL ORDER BY id DESC LIMIT 1`, [req.params.id, miembro.legajo_mecanico]);
                if (sesion) {
                    const horas = horasDesde(sesion.inicio);
                    await run(`UPDATE tiempos_actividad SET fin = CURRENT_TIMESTAMP WHERE id = ?`, [sesion.id]);
                    await run(`UPDATE actividad_mecanicos SET tiempo_real = tiempo_real + ? WHERE actividad_id = ? AND legajo_mecanico = ?`, [horas, req.params.id, miembro.legajo_mecanico]);
                }
            }
            await run(`UPDATE actividad_mecanicos SET estado = 'Finalizada' WHERE actividad_id = ? AND legajo_mecanico = ?`, [req.params.id, miembro.legajo_mecanico]);
        }

        await run(`UPDATE actividades SET estado = 'Cerrada por Jefe' WHERE id = ?`, [req.params.id]);
        await sincronizarEstadoActividad(req.params.id);
    });
    res.json({ status: 'Tarea interna cerrada' });
});

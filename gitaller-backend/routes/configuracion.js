...
router.put('/', requireAuth(['usuario_gestionar']), async (req, res) => {
    const { nombre_taller, hora_apertura, hora_cierre, hora_almuerzo_inicio, hora_almuerzo_fin, trabaja_corrido } = req.body;
    try {
        await run(`
            UPDATE configuracion SET 
            nombre_taller = ?, hora_apertura = ?, hora_cierre = ?, 
            hora_almuerzo_inicio = ?, hora_almuerzo_fin = ?, trabaja_corrido = ? 
            WHERE id = 1
        `, [nombre_taller, hora_apertura, hora_cierre, hora_almuerzo_inicio, hora_almuerzo_fin, trabaja_corrido ? 1 : 0]);
